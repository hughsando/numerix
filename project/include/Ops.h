...
inline void dot4Interlacedx3(float *dest, const float *alignedBias, const float *wABCD, const float *inSrc, int inDs, int n, Activation activation)
{
   #ifdef NUMERIX_SIMD
   if (true)
   {
      ...
      float32x4_t sum0 = Zero4f32;
      float32x4_t sum1 = sum0;
      float32x4_t sum2 = sum0;
      float32x4_t sum3 = sum0;
