...
typedef enum {
	...
	MVNC_NO_DRIVER = -99, // Numerix failed to load driver
} mvncStatus;
...
// Change name because mvnc looks for 'mvncOpenDevice' symbol when starting driver
mvncStatus nxMvncOpenDevice(const char *name, void **deviceHandle);
