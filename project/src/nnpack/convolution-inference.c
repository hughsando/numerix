...
static void compute_kernel_transform(
	const struct kernel_transform_context context[RESTRICT 1],
	...
	size_t output_channels_subblock_size,  size_t input_channels_block_increment)
{
   ...
   int ks = kernel_size.width * kernel_size.height;
	...
	for (size_t output_channels_subblock_offset = 0; output_channels_subblock_offset < output_channels_subblock_size; output_channels_subblock_offset += 1) {
		...
		transform_function(
			context->kernel + (output_channel*input_channels+input_channels_block_offset)*ks,
			(float *)( (char *)kernel_transform +
	...
	size_t input_channels_block_range,  size_t tiles_subblock_size)
{
   ...
   int inSize = input_size.height*input_size.width;
	...
	for (size_t tiles_subblock_offset = 0; tiles_subblock_offset < tiles_subblock_size; tiles_subblock_offset += 1) {
		...
		transform_function(
			context->input + (input_channel * inSize + input_y * input_size.width + input_x),
			(float *)( (char *)input_transform + (tiles_subblock_start * input_channels_block_size + input_channels_block_offset * tiles_subblock_size + tiles_subblock_offset) * tuple_size ),
			input_size.width,
...
static void compute_output_transform(
	const struct output_transform_context context[RESTRICT 1],
	...
	size_t output_channels_subblock_size,  size_t tiles_subblock_size)
{
   ...
   int outSize = output_size.height*output_size.width;
	...
	for (size_t tiles_subblock_offset = 0; tiles_subblock_offset < tiles_subblock_size; tiles_subblock_offset += 1) {
		...
		for (size_t output_channels_subblock_offset = 0; output_channels_subblock_offset < output_channels_subblock_size; output_channels_subblock_offset += 1) {
			...
			transform_function(
				...
				context->output + (output_channel*outSize + output_y*output_size.width + output_x),
				&bias[output_channel],
...
static void compute_tuple_multiplication(
	const struct tuple_multiplication_context context[RESTRICT 1],
	...
	size_t tiles_block_size,  size_t output_channels_subblock_size)
{
	...
	const char* input_transform  = (char *)context->input_transform +
		tiles_block_start * input_channels_block_size * tuple_size;
	const char* kernel_transform = (char *)context->kernel_transform +
		(output_channels_block_start + output_channels_subblock_start) * input_channels_block_size * tuple_size;
	char* output_transform       = (char *)context->output_transform +
...
static void compute_kernel_packing(
	const struct kernel_packing_context context[RESTRICT 1],
...
static void compute_input_packing(
	const struct input_packing_context context[RESTRICT 1],
   ...
   int inSize = input_size.height*input_size.width;
	...
	for (size_t output_image_subblock_offset = 0; output_image_subblock_offset < output_image_subblock_size; output_image_subblock_offset += 1) {
		...
		if ((input_x < input_size.width) && (input_y < input_size.height)) {
			packed_input[packed_index] = input[input_channel * inSize + input_y * input_size.width + input_x];
		} else {
...
static void compute_direct_convolution(
	const struct direct_convolution_context context[RESTRICT 1],
	...
	struct nnp_profile* profile)
{
	...
	switch (transform_strategy) {
		...
		case nnp_convolution_transform_strategy_reuse:
		{
			...
			void* output_transform = (char *)memory_block + input_transform_size;
			void* kernel_transform = (char *)memory_block + input_transform_size + output_transform_size;
