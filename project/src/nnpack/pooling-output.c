...
static void compute_max_pooling_forward__generic(
	const float *RESTRICT_VAL input_pointer,
	float *RESTRICT_VAL output_pointer,
	size_t input_height,
	size_t input_width,
	size_t padding_top,
	size_t padding_left,
			...
			for (size_t i = 0; i < pooling_height; i++) {
				...
				if (s < input_height) {
					for (size_t j = 0; j < pooling_width; j++) {
						...
						if (t < input_width) {
							//v = maxf(input[s][t], v);
							v = maxf(input_pointer[s*input_width+t], v);
						}
					}
				}
			}
			//output[y][x] = v;
			output_pointer[y*output_width+x] = v;
		}
	}
}
...
static void compute_pooling_output(
	const struct pooling_context context[RESTRICT 1],
	size_t sample, size_t channel)
{
	...
	pooling_function(
		...
		context->input_pointer + (sample*channels + channel)*(input_size.height * input_size.width),
		context->output_pointer + (sample*channels + channel)*(output_size.height * output_size.width),
