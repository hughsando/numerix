   ...
   int ks = kernel_size.width * kernel_size.height;
	...
	for (size_t output_channels_subblock_offset = 0; output_channels_subblock_offset < output_channels_subblock_size; output_channels_subblock_offset += 1) {
		...
		transform_function(
			...
			((const float *)context->kernel) + (output_channel*input_channels + input_channel)*ks,
			kernel_transform +
	...
	size_t input_channel_range, size_t batch_subblock_size)
{
   ...
   int inSize = input_size.width * input_size.height;
	...
	for (size_t batch_subblock_offset = 0; batch_subblock_offset < batch_subblock_size; batch_subblock_offset += 1) {
		...
		transform_function(
			context->input + (sample*input_channels + input_channel)*inSize,
			input_transform +
	...
	size_t sample_range, size_t output_channels_subblock_size)
{
   ...
   int outSize = output_size.width * output_size.height;
	...
	for (size_t output_channels_subblock_offset = 0; output_channels_subblock_offset < output_channels_subblock_size; output_channels_subblock_offset += 1) {
		...
		transform_function(
			...
			(float *)context->output + (sample*output_channels + output_channel)*outSize,
			&bias[output_channel],
